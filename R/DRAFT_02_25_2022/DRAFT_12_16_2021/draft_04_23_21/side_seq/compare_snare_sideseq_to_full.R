...
load(here("output/snare_sideseq/full_side_seq_snare_1500.RData"))
...
methods <- c("Cell Subgroup Strata", 
             "Random Cells")
...
for(r in ref_set_size) {
  ...
  for(s in 1:5) {
    ...
    nn_consistency[(2*i+1), (2+s)] = avg_embed
    nn_consistency[(2*i+2), (2+s)] = avg_rand
    
    nn_consistency[(2*i+1), (7+s)] = side_seq_ref_cell_embed[[3]]
    nn_consistency[(2*i+2), (7+s)] = side_seq_ref_rand[[3]]
...
nn_consistency_long$method <-
  forcats::fct_relevel(
    nn_consistency_long$method,
    c("Cell Subgroup Strata (Max)", "Cell Subgroup Strata (Avg)", "Cell Subgroup Strata (Min)",
...
summary(
  glm(compute_time ~ ref_set_size,
      data = compute_time_long %>% filter(method == "Cell Subgroup Strata (Avg)")))
summary(
  glm(compute_time ~ ref_set_size,
      data = compute_time_long %>% filter(method == "Random Cells (Avg)")))
...
compute_time_long %>%
  ...
  geom_bar(stat = "identity", position = "dodge", col = "black", 
           width=16, alpha = 0.75) + 
  geom_abline(slope = 5.98/60, intercept = 39.05 / 60, 
              col = "black", alpha = 0.5, lty = 2) +
  geom_abline(slope = 23.95/60, intercept = 575.92 / 60, 
              col = "black", alpha = 0.5, lty = 2) +
  theme_bw() + 
  scale_x_continuous(name = "Reference Set Size", 
                     breaks=c(25, 50, 75, 100,
...
purity_data <- data.frame(run_info = rep("", length(ref_set_size) * 5),
...
for(r in ref_set_size) {
  print(r)
  for(s in 1:5) {
    load(here("output/snare_sideseq/run" %p% s %p% "_" %p% r %p% 
                "cell_embed_ref_set_snare_1500.RData"))
    
    purity_data[i,] <- c(r %p% "_" %p% s, side_seq_ref_cell_embed$purity_scores[-1])
...
ggsave(here("output/figures/purity_sideref_snare.png"),
       width = 8, height = 5)
