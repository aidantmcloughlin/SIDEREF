...
SIDEseqIteratedRand <- function(expr_matrix, 
  ...
  getSimilarityVal <- function(x) {
    ...
    if((N-2) <= n_cells_samp) {
      ...
      cell_pairs_subset <- 
        rbind(c(rep(x[1], n_cells_samp), rep(x[2], n_cells_samp)),
          rep(sample(setdiff(seq_len(N), c(x[1], x[2])), 
                     n_cells_samp), 2))  
        }
    ...
    return(similarityMeasure(i = x[1], j = x[2], 
                             N = min(N, n_cells_samp+2), 
...
# pairs <- combn(N, 2)
# 
# sample_mat <- do.call(cbind,
#   lapply(seq_len(ncol(pairs)), function(x) {
#     samp <- sample(setdiff(1:N, pairs[,x]), s, replace = FALSE)
#     return(cbind(rbind(pairs[1,x], samp), rbind(pairs[2,x], samp)))
#   }))
# 
# test2 <-
#   unique(t(apply(sample_mat, 2, sort)))
