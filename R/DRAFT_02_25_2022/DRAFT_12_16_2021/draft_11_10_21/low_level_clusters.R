...
cell_idx_shuf <- sample(seq_len(ncol(data_full)), replace=FALSE)

cell_idx_splits <- split(cell_idx_shuf, 
                         cut(seq_along(cell_idx_shuf), FOLDS, 
                             labels = FALSE))
...
if(RUN_DIST_AND_CLUST) {
  ...
  for(i in seq_len(length(tm_filenames))) {
    ...
    names(tm_clust_by_file)[i] <- tm_filenames[i]
  }
  ...
  save(full_dist_res, 
       file = here("output/tab_muris_sc/dist_res/full_tab_muris_dist_res_" %p% 
                     data_pct %p% "_" %p% file_tag %p% ".RData"))
...
load(here("output/tab_muris_sc/dist_res/full_tab_muris_dist_res_" %p% 
                   data_pct %p% "_" %p% file_tag %p% ".RData"))
...
## Avg Sil on full distance matrix
avg_sil_full_dist <-
...
## only on annotated cells
annotated_cells <- which(!is.na(meta_data_full$cell_type))

tab_muris_hclust_res_with_celltype <-
  lapply(full_res$dist_list,
         function(x) {
           res <- 
             RunHClustPipeline(
               clusters = seq_len(nrow(meta_data_full[annotated_cells,])), 
               subg_inds = as.numeric(factor(meta_data_full[annotated_cells,]$tissue)),
               hclust_dissim_mat = x[annotated_cells, annotated_cells], 
               p_dissim_mat      = full_res$dist_list$pca_dist_25[annotated_cells, 
                                                                  annotated_cells])
         })
