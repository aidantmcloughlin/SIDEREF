  ...
  if(similarity_method == "n_intersect") {
    if(side_seq_method == "simultaneous") {
      ...
      similarityMeasure <- function(i, j, N, J,
                                    ...
                                    d_e_mat,
                                    contribs_val,
                                    mult_list_trunc = 3 ## in case of bootstrap how much to extend each list?
                                    ) {
        ...
        for(r in seq_len(R)) {
          if(ref_set[r] %in% c(i,j)){
            ...
            if(!is.null(contribs_val)) {
              sim_score[[2]][inter] <- sim_score[[2]][inter] + R_trim_recip
            }
          }
        ...
        if(!is.null(contribs_val)) {
          gene_contribs <-
            order(-sim_score[[2]], sample(seq_len(length(gene_contrib_vec))))
          
          ## trim non-contributors from the list
          n_contributors <- sum(sim_score[[2]] > 1e-9)
          gene_contribs <- gene_contribs[1:min(n_contributors, contribs_val * mult_list_trunc)]}
